...
import { Track, Playlist, AppSettings, AppState } from './src/types';
import { getAudioPlayer, PlayerStatus } from './src/services/audio-player';
import { StorageService } from './src/services/storage';
import { Splash } from './src/screens/Splash';
import { Home } from './src/screens/Home';
import { Playlist as PlaylistScreen } from './src/screens/Playlist';
import { NowPlaying } from './src/screens/NowPlaying';
import { FileBrowser } from './src/screens/FileBrowser';
import { Settings } from './src/screens/Settings';
import { GestureTutorial } from './src/screens/GestureTutorial';
...
type PlaylistProps = NativeStackScreenProps<RootStackParamList, 'Playlist'>;
...
type GestureTutorialProps = NativeStackScreenProps<RootStackParamList, 'GestureTutorial'>;
...
export default function App() {
    ...
    const init = async () => {
      try {
        ...
        if (savedState && savedState.currentTrackId) {
          const track = playlists[0]?.tracks.find((t: { id: any }) => t.id === savedState.currentTrackId);
          if (track) {
  ...
  useEffect(() => {
    ...
    const unsubscribeError = audioPlayer.onError((error: { message: any }) => {
      console.error('Audio player error:', error);
      StorageService.addLog(`Player error: ${error.message}`);
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: `Player error: ${error.message}`,
      });
    });
  ...
  const loadAndPlayTrack = useCallback(async (track: Track) => {
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: `Failed to load track: ${error}`,
      });
    }
  }, [audioPlayer]);
  ...
  }, [tracks, currentTrack, audioPlayer, shuffle, loadAndPlayTrack]);
  ...
  const handleRemoveTrack = useCallback((trackId: string) => {
    ...
    Toast.show({
      type: 'info',
      text1: 'Track Removed',
      text2: `Track ${trackId} removed from playlist`,
    });
      ...
      <NavigationContainer>
        <Stack.Navigator initialRouteName="Splash" screenOptions={{ headerShown: false }}>
          <Stack.Screen name="Splash">
            {(props: SplashProps) => <Splash {...props} onComplete={() => props.navigation.navigate('Home')} />}
          </Stack.Screen>
          ...
          <Stack.Screen name="FileBrowser">
            {(props: FileBrowserProps) => (
              <FileBrowser {...props} onAddTracks={handleAddTracks} onNavigate={props.navigation.navigate} />
            )}
          </Stack.Screen>
          ...
          <Stack.Screen name="GestureTutorial">
            {(props: GestureTutorialProps) => <GestureTutorial {...props} onNavigate={props.navigation.navigate} />}
          </Stack.Screen>
          <Stack.Screen name="About">
            {(props: AboutProps) => <About {...props} onNavigate={props.navigation.navigate} />}
          </Stack.Screen>
        </Stack.Navigator>
      </NavigationContainer>
      <Toast />
